...
class Collected {
  /// Returns a new [Collected] instance.
  Collected({
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is Collected &&
  ...
  @override
  String toString() => 'Collected[balanceUsedCents=$balanceUsedCents, cardChargedCents=$cardChargedCents, creditUsedCents=$creditUsedCents, invoice=$invoice, paid=$paid, processorRef=$processorRef, reason=$reason]';
  ...
  /// Returns a new [Collected] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static Collected? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "Collected[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "Collected[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Collected> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <Collected>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = Collected.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, Collected> mapFromJson(dynamic json) {
    final map = <String, Collected>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Collected.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of Collected-objects as value to a dart map
  static Map<String, List<Collected>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<Collected>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = Collected.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
