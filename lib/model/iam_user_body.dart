...
class IamUserBody {
  /// Returns a new [IamUserBody] instance.
  IamUserBody({
    this.accessKey,
    this.accessSecret,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is IamUserBody &&
  ...
  /// Returns a new [IamUserBody] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static IamUserBody? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "IamUserBody[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "IamUserBody[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<IamUserBody> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <IamUserBody>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = IamUserBody.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, IamUserBody> mapFromJson(dynamic json) {
    final map = <String, IamUserBody>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = IamUserBody.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of IamUserBody-objects as value to a dart map
  static Map<String, List<IamUserBody>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<IamUserBody>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = IamUserBody.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
