...
class O11yVolumeListRecord {
  /// Returns a new [O11yVolumeListRecord] instance.
  ...
  @override
  String toString() => 'O11yVolumeListRecord[meta=$meta, persistentVolumeClaimName=$persistentVolumeClaimName, volumeAvailable=$volumeAvailable, volumeCapacity=$volumeCapacity, volumeInodes=$volumeInodes, volumeInodesFree=$volumeInodesFree, volumeInodesUsed=$volumeInodesUsed, volumeUsage=$volumeUsage]';
  ...
  /// Returns a new [O11yVolumeListRecord] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static O11yVolumeListRecord? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "O11yVolumeListRecord[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "O11yVolumeListRecord[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<O11yVolumeListRecord> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <O11yVolumeListRecord>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = O11yVolumeListRecord.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, O11yVolumeListRecord> mapFromJson(dynamic json) {
    final map = <String, O11yVolumeListRecord>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = O11yVolumeListRecord.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of O11yVolumeListRecord-objects as value to a dart map
  static Map<String, List<O11yVolumeListRecord>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<O11yVolumeListRecord>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = O11yVolumeListRecord.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
