...
class CaptableShareClassAmend {
  /// Returns a new [CaptableShareClassAmend] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is CaptableShareClassAmend &&
  ...
  @override
  String toString() => 'CaptableShareClassAmend[boardApprovalDate=$boardApprovalDate, classType=$classType, conversionRights=$conversionRights, convertsToShareClassId=$convertsToShareClassId, id=$id, initialSharesAuthorized=$initialSharesAuthorized, liquidationPreferenceMultiple=$liquidationPreferenceMultiple, name=$name, parValue=$parValue, participationCapMultiple=$participationCapMultiple, pricePerShare=$pricePerShare, seniority=$seniority, stockholderApprovalDate=$stockholderApprovalDate, votesPerShare=$votesPerShare]';
  ...
  /// Returns a new [CaptableShareClassAmend] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static CaptableShareClassAmend? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CaptableShareClassAmend[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CaptableShareClassAmend[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return CaptableShareClassAmend(
  ...
  static List<CaptableShareClassAmend> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <CaptableShareClassAmend>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = CaptableShareClassAmend.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, CaptableShareClassAmend> mapFromJson(dynamic json) {
    final map = <String, CaptableShareClassAmend>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CaptableShareClassAmend.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of CaptableShareClassAmend-objects as value to a dart map
  static Map<String, List<CaptableShareClassAmend>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<CaptableShareClassAmend>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = CaptableShareClassAmend.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
