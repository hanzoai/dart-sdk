...
class InvoiceOut {
  /// Returns a new [InvoiceOut] instance.
  InvoiceOut({
    this.amountDueCents,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is InvoiceOut &&
  ...
  @override
  String toString() => 'InvoiceOut[amountDueCents=$amountDueCents, amountPaidCents=$amountPaidCents, createdAt=$createdAt, currency=$currency, customerEmail=$customerEmail, id=$id, lines=$lines, number=$number, paymentRef=$paymentRef, status=$status, subtotalCents=$subtotalCents, userId=$userId]';
  ...
  /// Returns a new [InvoiceOut] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static InvoiceOut? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "InvoiceOut[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "InvoiceOut[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<InvoiceOut> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <InvoiceOut>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = InvoiceOut.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, InvoiceOut> mapFromJson(dynamic json) {
    final map = <String, InvoiceOut>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = InvoiceOut.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of InvoiceOut-objects as value to a dart map
  static Map<String, List<InvoiceOut>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<InvoiceOut>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = InvoiceOut.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
