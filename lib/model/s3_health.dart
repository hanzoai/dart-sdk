...
class S3Health {
  /// Returns a new [S3Health] instance.
  S3Health({
    this.error,
    this.presign,
    this.ready,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is S3Health &&
    other.error == error &&
  ...
  @override
  String toString() => 'S3Health[error=$error, presign=$presign, ready=$ready, service=$service, status=$status]';
  ...
  /// Returns a new [S3Health] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static S3Health? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "S3Health[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "S3Health[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<S3Health> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <S3Health>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = S3Health.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, S3Health> mapFromJson(dynamic json) {
    final map = <String, S3Health>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = S3Health.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of S3Health-objects as value to a dart map
  static Map<String, List<S3Health>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<S3Health>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = S3Health.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
