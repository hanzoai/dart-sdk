...
class ReadingView {
  /// Returns a new [ReadingView] instance.
  ReadingView({
    this.account,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ReadingView &&
  ...
  /// Returns a new [ReadingView] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ReadingView? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ReadingView[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ReadingView[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ReadingView> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ReadingView>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ReadingView.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ReadingView> mapFromJson(dynamic json) {
    final map = <String, ReadingView>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ReadingView.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of ReadingView-objects as value to a dart map
  static Map<String, List<ReadingView>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ReadingView>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ReadingView.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
