...
class TeamApi {
  ...
  Future<Response> deleteTeamFilesBySpaceByFilenameWithHttpInfo(String space, String filename, { String? file, }) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/team/files/{space}/{filename}'
  ...
  /// * [String] file:
  ///   File is the blob id, and wins over the path segment when both are present.
  Future<void> deleteTeamFilesBySpaceByFilename(String space, String filename, { String? file, }) async {
    final response = await deleteTeamFilesBySpaceByFilenameWithHttpInfo(space, filename,  file: file, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  }
  ...
  ///
  /// * [String] filename (required):
  Future<Response> getTeamFilesBySpaceByFilenameWithHttpInfo(String space, String filename,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/team/files/{space}/{filename}'
      .replaceAll('{space}', space)
      .replaceAll('{filename}', filename);
  ...
  ///
  /// * [String] filename (required):
  Future<MultipartFile?> getTeamFilesBySpaceByFilename(String space, String filename,) async {
    final response = await getTeamFilesBySpaceByFilenameWithHttpInfo(space, filename,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Response> getTeamRoomsByIdMessagesWithHttpInfo(String id, { String? space, }) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/team/rooms/{id}/messages'
      .replaceAll('{id}', id);
    ...
    if (space != null) {
      queryParams.addAll(_queryParams('', 'space', space));
    }
  ...
  /// * [String] space:
  ///   Space names the space holding the room, and is required for the reason the bind op requires it: a room id is unique within a space and not across the org, so searching every space for a match would make the answer depend on iteration order.
  Future<TeamMessages?> getTeamRoomsByIdMessages(String id, { String? space, }) async {
    final response = await getTeamRoomsByIdMessagesWithHttpInfo(id,  space: space, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'TeamMessages',) as TeamMessages;
  ...
  ///
  /// * [MultipartFile] body:
  Future<Response> postTeamFilesBySpaceWithHttpInfo(String space, { MultipartFile? body, }) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/team/files/{space}'
      .replaceAll('{space}', space);
  ...
  ///
  /// * [MultipartFile] body:
  Future<MultipartFile?> postTeamFilesBySpace(String space, { MultipartFile? body, }) async {
    final response = await postTeamFilesBySpaceWithHttpInfo(space,  body: body, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  /// Parameters:
  ///
  /// * [TeamRoomNew] teamRoomNew (required):
  Future<Response> postTeamRoomsWithHttpInfo(TeamRoomNew teamRoomNew,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/team/rooms';

    // ignore: prefer_final_locals
    Object? postBody = teamRoomNew;
  ...
  ///
  /// * [TeamRoomNew] teamRoomNew (required):
  Future<TeamRoom?> postTeamRooms(TeamRoomNew teamRoomNew,) async {
    final response = await postTeamRoomsWithHttpInfo(teamRoomNew,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  ///
  /// * [TeamMessageWrite] teamMessageWrite (required):
  Future<Response> postTeamRoomsByIdMessagesWithHttpInfo(String id, TeamMessageWrite teamMessageWrite,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/team/rooms/{id}/messages'
      .replaceAll('{id}', id);

    // ignore: prefer_final_locals
    Object? postBody = teamMessageWrite;
  ...
  ///
  /// * [TeamMessageWrite] teamMessageWrite (required):
  Future<TeamMessage?> postTeamRoomsByIdMessages(String id, TeamMessageWrite teamMessageWrite,) async {
    final response = await postTeamRoomsByIdMessagesWithHttpInfo(id, teamMessageWrite,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'TeamMessage',) as TeamMessage;
