...
class ValidatorApi {
  ValidatorApi([ApiClient? apiClient]) : apiClient = apiClient ?? defaultApiClient;
  ...
  Future<Response> getValidatorWithHttpInfo({ String? limit, }) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/validator';
  ...
  /// * [String] limit:
  ///   Limit is how many slots to return, as a decimal string in the `?limit=` query. Absent, unparseable or non-positive means 200; over 1000 is clamped to 1000. It is a string rather than a number because the parse that has always served this route trims surrounding whitespace, and one parse rule is better than two.
  Future<ValidatorList?> getValidator({ String? limit, }) async {
    final response = await getValidatorWithHttpInfo( limit: limit, );
  ...
  Future<Response> getValidatorByTokenidWithHttpInfo(String tokenId,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/validator/{tokenId}'
  ...
  /// * [String] tokenId (required):
  ///   TokenID is the slot's GenesisNFT token id, from the path, as a decimal string. A value that is not a positive integer is 400. It is a string rather than a number because the parse that has always served this route trims surrounding whitespace, and one parse rule is better than two.
  Future<SlotView?> getValidatorByTokenid(String tokenId,) async {
    final response = await getValidatorByTokenidWithHttpInfo(tokenId,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Response> getValidatorChallengeWithHttpInfo({ String? tokenId, }) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/validator/challenge';
  ...
  /// * [String] tokenId:
  ///   TokenID is the Validator-tier GenesisNFT token id, as a decimal string in the `?tokenId=` query. A value that is not a positive integer is 400. It is a string rather than a number because the parse that has always served this route trims surrounding whitespace, and one parse rule is better than two.
  Future<ChallengeView?> getValidatorChallenge({ String? tokenId, }) async {
    final response = await getValidatorChallengeWithHttpInfo( tokenId: tokenId, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Response> postValidatorWithHttpInfo(ValidatorClaim validatorClaim,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/validator';
  ...
  ///
  /// * [ValidatorClaim] validatorClaim (required):
  Future<SlotView?> postValidator(ValidatorClaim validatorClaim,) async {
    final response = await postValidatorWithHttpInfo(validatorClaim,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
