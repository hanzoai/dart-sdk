...
class ExperimentApi {
  ExperimentApi([ApiClient? apiClient]) : apiClient = apiClient ?? defaultApiClient;
  ...
  Future<Response> getExperimentWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/v1/experiment';
  ...
  ///
  /// Is every experiment in the caller's org, with its variants, status and decision, ordered by project then id.  Scoped to the org resolved from the validated principal — a distinct org is a distinct physical store, so no query here can reach another tenant's rows — and further narrowed to the caller's project scope when the credential carries one. A principal with NO project scope sees the org's experiments across all of its projects, which is the answer a reader most often expects to be filtered and is not.  Requires a validated principal; refuses without one rather than answering an empty list.
  Future<ExperimentList?> getExperiment() async {
    final response = await getExperimentWithHttpInfo();
  ...
  Future<Response> getExperimentByIdWithHttpInfo(String id,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/experiment/{id}'
      .replaceAll('{id}', id);
  ...
  /// * [String] id (required):
  ///   ID is the experiment the URL names.
  Future<Trial?> getExperimentById(String id,) async {
    final response = await getExperimentByIdWithHttpInfo(id,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Response> getExperimentByIdAssignWithHttpInfo(String id, String subject, { String? props, }) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/experiment/{id}/assign'
      .replaceAll('{id}', id);
  ...
  /// * [String] props:
  ///   Props is a JSON object of person properties for targeting. A value that is not valid JSON is dropped rather than refused, so a malformed one changes the bucketing without saying so.
  Future<Assignment?> getExperimentByIdAssign(String id, String subject, { String? props, }) async {
    final response = await getExperimentByIdAssignWithHttpInfo(id, subject,  props: props, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> getExperimentHealthWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/v1/experiment/health';
  ...
  ///
  /// Is whether the experiments subsystem is mounted and serving in this process.  It answers unconditionally. It proves exactly one thing — that this binary registered the experiments routes and is dispatching them — and deliberately no more: it reads no principal, opens no per-org registry, and touches neither the flags engine nor the analytics plane, so a 200 here says nothing about whether a given tenant's store will open or whether an analysis can run. It is the only route on this surface that needs no org.  The static path is registered ahead of the /:id read, so it always wins the first-match scan. \"health\" is a legal experiment id, which means an experiment created under that id can never be fetched by id — pick another.
  Future<Health?> getExperimentHealth() async {
    final response = await getExperimentHealthWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Response> postExperimentWithHttpInfo(CreateBody createBody,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/experiment';
  ...
  ///
  /// * [CreateBody] createBody (required):
  Future<Trial?> postExperiment(CreateBody createBody,) async {
    final response = await postExperimentWithHttpInfo(createBody,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Response> postExperimentByIdAnalyzeWithHttpInfo(String id, AnalyzeQuery analyzeQuery,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/experiment/{id}/analyze'
      .replaceAll('{id}', id);
  ...
  ///
  /// * [AnalyzeQuery] analyzeQuery (required):
  Future<Analysis?> postExperimentByIdAnalyze(String id, AnalyzeQuery analyzeQuery,) async {
    final response = await postExperimentByIdAnalyzeWithHttpInfo(id, analyzeQuery,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Response> postExperimentByIdDecideWithHttpInfo(String id, DecideBody decideBody,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/experiment/{id}/decide'
      .replaceAll('{id}', id);
  ...
  ///
  /// * [DecideBody] decideBody (required):
  Future<Trial?> postExperimentByIdDecide(String id, DecideBody decideBody,) async {
    final response = await postExperimentByIdDecideWithHttpInfo(id, decideBody,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
