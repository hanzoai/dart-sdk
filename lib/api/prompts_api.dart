...
class PromptsApi {
  PromptsApi([ApiClient? apiClient]) : apiClient = apiClient ?? defaultApiClient;
  ...
  Future<Response> deletePromptsByNameWithHttpInfo(String name,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/prompts/{name}'
  ...
  /// * [String] name (required):
  ///   Name is the prompt to act on, from the path.
  Future<void> deletePromptsByName(String name,) async {
    final response = await deletePromptsByNameWithHttpInfo(name,);
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> getPromptsWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/v1/prompts';
  ...
  ///
  /// List returns the caller org's prompt library as one row per prompt: its name, type, every version number it has, its taxonomy and when it last changed. The template bodies are deliberately absent — fetch one prompt to read its text.
  Future<PromptList?> getPrompts() async {
    final response = await getPromptsWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  ...
  /// * [String] name (required):
  ///   Name is the prompt to act on, from the path.
  Future<Response> getPromptsByNameWithHttpInfo(String name,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/prompts/{name}'
      .replaceAll('{name}', name);
  ...
  /// * [String] name (required):
  ///   Name is the prompt to act on, from the path.
  Future<PromptDetail?> getPromptsByName(String name,) async {
    final response = await getPromptsByNameWithHttpInfo(name,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> getPromptsCatalogWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/v1/prompts/catalog';
  ...
  ///
  /// Catalog returns the read-only starter prompt library shipped with the binary — reference content every tenant sees the same, NOT the caller's own prompts and never mixed into them. An org's library stays honestly empty until someone explicitly imports a starter, which is an ordinary POST /v1/prompts. Entries that would fail the create guards are dropped, so everything offered here can actually be imported.
  Future<CatalogList?> getPromptsCatalog() async {
    final response = await getPromptsCatalogWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> getPromptsMetricsWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/v1/prompts/metrics';
  ...
  ///
  /// Metrics returns real per-prompt statistics for the caller's org: how many versions each prompt has, which one is current, and when it was created and last changed. Every number is counted from the store — nothing here is estimated or fabricated.
  Future<MetricList?> getPromptsMetrics() async {
    final response = await getPromptsMetricsWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Response> postPromptsWithHttpInfo(PromptReq promptReq,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/prompts';
  ...
  ///
  /// * [PromptReq] promptReq (required):
  Future<PromptDetail?> postPrompts(PromptReq promptReq,) async {
    final response = await postPromptsWithHttpInfo(promptReq,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
