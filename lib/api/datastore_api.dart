...
class DatastoreApi {
  DatastoreApi([ApiClient? apiClient]) : apiClient = apiClient ?? defaultApiClient;
  ...
  Future<Response> deleteDatastoreByNameWithHttpInfo(String name,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/datastore/{name}'
  ...
  /// * [String] name (required):
  ///   Name is the resource's org-unique slug, from the path. Lower-cased and trimmed before lookup, exactly as it was at create.
  Future<void> deleteDatastoreByName(String name,) async {
    final response = await deleteDatastoreByNameWithHttpInfo(name,);
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> getDatastoreWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/v1/datastore';
  ...
  ///
  /// Lists the caller org's Hanzo Datastore warehouses. Each one is a DEDICATED analytical instance the org alone runs, so the host is that instance's own in-cluster Service and the port is its HTTP port, 8123.
  Future<List<ProvisionedSummary>?> getDatastore() async {
    final response = await getDatastoreWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  ...
  /// * [String] name (required):
  ///   Name is the resource's org-unique slug, from the path. Lower-cased and trimmed before lookup, exactly as it was at create.
  Future<Response> getDatastoreByNameWithHttpInfo(String name,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/datastore/{name}'
      .replaceAll('{name}', name);
  ...
  /// * [String] name (required):
  ///   Name is the resource's org-unique slug, from the path. Lower-cased and trimmed before lookup, exactly as it was at create.
  Future<ProvisionedResource?> getDatastoreByName(String name,) async {
    final response = await getDatastoreByNameWithHttpInfo(name,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Response> postDatastoreWithHttpInfo({ ProvisionRequest? provisionRequest, }) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/datastore';
  ...
  ///
  /// * [ProvisionRequest] provisionRequest:
  Future<ProvisionResult?> postDatastore({ ProvisionRequest? provisionRequest, }) async {
    final response = await postDatastoreWithHttpInfo( provisionRequest: provisionRequest, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
