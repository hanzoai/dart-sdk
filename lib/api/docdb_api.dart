...
class DocdbApi {
  DocdbApi([ApiClient? apiClient]) : apiClient = apiClient ?? defaultApiClient;
  ...
  Future<Response> deleteDocdbByNameWithHttpInfo(String name,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/docdb/{name}'
  ...
  /// * [String] name (required):
  ///   Name is the resource's org-unique slug, from the path. Lower-cased and trimmed before lookup, exactly as it was at create.
  Future<void> deleteDocdbByName(String name,) async {
    final response = await deleteDocdbByNameWithHttpInfo(name,);
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> getDocdbWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/v1/docdb';
  ...
  ///
  /// ListDocDB lists the caller org's Hanzo DocDB document databases. Each one is a DEDICATED FerretDB instance the org alone runs, speaking the MongoDB wire protocol, so the host is that instance's own in-cluster Service and the port is 27017.
  Future<List<ProvisionedSummary>?> getDocdb() async {
    final response = await getDocdbWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  ...
  /// * [String] name (required):
  ///   Name is the resource's org-unique slug, from the path. Lower-cased and trimmed before lookup, exactly as it was at create.
  Future<Response> getDocdbByNameWithHttpInfo(String name,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/docdb/{name}'
      .replaceAll('{name}', name);
  ...
  /// * [String] name (required):
  ///   Name is the resource's org-unique slug, from the path. Lower-cased and trimmed before lookup, exactly as it was at create.
  Future<ProvisionedResource?> getDocdbByName(String name,) async {
    final response = await getDocdbByNameWithHttpInfo(name,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Response> postDocdbWithHttpInfo({ ProvisionRequest? provisionRequest, }) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/docdb';
  ...
  ///
  /// * [ProvisionRequest] provisionRequest:
  Future<ProvisionResult?> postDocdb({ ProvisionRequest? provisionRequest, }) async {
    final response = await postDocdbWithHttpInfo( provisionRequest: provisionRequest, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
