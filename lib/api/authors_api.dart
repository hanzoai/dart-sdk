...
class AuthorsApi {
  AuthorsApi([ApiClient? apiClient]) : apiClient = apiClient ?? defaultApiClient;
  ...
  Future<Response> getAuthorsWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/v1/authors';
  ...
  ///
  /// Returns the caller's author-program dashboard: enrolment status, linked forge login, verified repositories and owner-wide claims, recorded deploys, accrued / pending / paid royalty, and the payout history.  It answers ONE OF TWO SHAPES from this address. An org that has never connected gets {\"isAuthor\": false, \"defaultShareBps\", \"badgeBase\"} — an honest \"not enrolled\" rather than a 404, so the console can render the connect form. An enrolled org gets the dashboard: isAuthor, id, status, githubLogin, verified, verifyCode, verifyFile, verifySnippet, shareBps, badgeBase, repos, orgs, deploys, accruedCents, pendingCents, paidCents, payouts and ledger.  For an APPROVED author this read ALSO runs the accrual sweep opportunistically, so the dashboard is self-updating. That is why the royalty AUDIT lives at its own address: an audit must not move the money it is auditing.
  Future<Map<String, Object>?> getAuthors() async {
    final response = await getAuthorsWithHttpInfo();
  ...
  Future<Response> getAuthorsBasisWithHttpInfo({ String? period, }) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/authors/basis';
  ...
  /// * [String] period:
  ///   Period is the UTC accrual month, YYYY-MM. Empty means every period; any other shape is refused with 400, because the period is echoed back and used as a SQL filter and is only ever accepted in the one form the accrual latch mints.
  Future<Map<String, Object>?> getAuthorsBasis({ String? period, }) async {
    final response = await getAuthorsBasisWithHttpInfo( period: period, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Response> postAuthorsConnectWithHttpInfo(ConnectRequest connectRequest,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/authors/connect';
  ...
  ///
  /// * [ConnectRequest] connectRequest (required):
  Future<Enrolment?> postAuthorsConnect(ConnectRequest connectRequest,) async {
    final response = await postAuthorsConnectWithHttpInfo(connectRequest,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Response> postAuthorsDeploysRecordWithHttpInfo(DeployRequest deployRequest,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/authors/deploys/record';
  ...
  ///
  /// * [DeployRequest] deployRequest (required):
  Future<DeployRecord?> postAuthorsDeploysRecord(DeployRequest deployRequest,) async {
    final response = await postAuthorsDeploysRecordWithHttpInfo(deployRequest,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Response> postAuthorsReposVerifyWithHttpInfo(VerifyRequest verifyRequest,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/authors/repos/verify';
  ...
  ///
  /// * [VerifyRequest] verifyRequest (required):
  Future<Claim?> postAuthorsReposVerify(VerifyRequest verifyRequest,) async {
    final response = await postAuthorsReposVerifyWithHttpInfo(verifyRequest,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
