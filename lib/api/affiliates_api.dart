...
class AffiliatesApi {
  AffiliatesApi([ApiClient? apiClient]) : apiClient = apiClient ?? defaultApiClient;
  ...
  Future<Response> getAffiliatesWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/v1/affiliates';
  ...
  ///
  /// Answers the caller org's OWN affiliate standing: status, referral code and share link, commission rate, how many orgs it has referred, and its lifetime accrued, still-pending and already-paid commission in integer cents, with its payout history.  An org that never applied gets an honest `isAffiliate:false` and the default rate rather than a 404 — the console renders the apply form off that answer.  The affiliate is resolved from the VALIDATED org, never from a field, so this can only ever read the caller's own row; without a principal it is refused. It is a PURE READ: nothing accrues until the sweep runs. Commission is earned on Hanzo's MARGIN, never on the referred customer's bill, so nothing here changes what that customer pays.
  Future<AffiliateStanding?> getAffiliates() async {
    final response = await getAffiliatesWithHttpInfo();
    ...
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'AffiliateStanding',) as AffiliateStanding;
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> getAffiliatesLeaderboardWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/v1/affiliates/leaderboard';
  ...
  ///
  /// Answers the top affiliates by lifetime accrued commission, shown by OPT-IN HANDLE with aggregate figures only, plus the caller's own exact rank.  It never discloses an org identity and never a referred org's usage. An affiliate that has set no handle still OCCUPIES its rank but is not listed — so opting out hides the name, not the position, and the visible board must not be read as a complete roster.  The caller's own row carries its exact GLOBAL rank, computed over the whole approved set rather than over the page, so it is right well outside the top of the board. Only an approved affiliate has a rank. Requires a validated principal; a signed-in non-affiliate may read the board but gets no personal row.
  Future<AffiliateBoard?> getAffiliatesLeaderboard() async {
    final response = await getAffiliatesLeaderboardWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'AffiliateBoard',) as AffiliateBoard;
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> getAffiliatesMeWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/v1/affiliates/me';
  ...
  ///
  /// Answers the richer self-view: the same lifetime accrued, pending and paid commission and payout history, plus the caller's downline broken out by upline LEVEL — direct, second, third — each with the rate paid at that level and how many orgs sit there.  Commission is MULTI-LEVEL: a referred org's spend pays up its referral chain, three levels deep and no further. The direct level is the affiliate's own negotiated rate; the second and third are platform-wide switches, read live, so the schedule shown is the one actually in force rather than one compiled in. A caller that has not applied still gets that schedule alongside `isAffiliate:false`, so the console can show what it would earn.  Scoped to the validated org and nothing else, and refused without a principal. A PURE READ — it reports the downline but accrues nothing.
  Future<AffiliateSelf?> getAffiliatesMe() async {
    final response = await getAffiliatesMeWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'AffiliateSelf',) as AffiliateSelf;
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> getAffiliatesMeEarningsWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/v1/affiliates/me/earnings';
  ...
  ///
  /// Answers the caller's own commission ledger: per period, the margin it earned against and the commission taken from that margin; and per referred org, that referral's aggregate contribution. Integer cents throughout.  The per-org view deliberately carries the affiliate's OWN earned share and NOT the referred org's spend or margin. An affiliate is entitled to what it earned, not to a restatement of its customer's usage — the period view is where the margin base appears, aggregated across every referral.  Scoped server-side to the validated caller's affiliate; a caller that is not one gets `isAffiliate:false`.
  Future<AffiliateEarnings?> getAffiliatesMeEarnings() async {
    final response = await getAffiliatesMeEarningsWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'AffiliateEarnings',) as AffiliateEarnings;
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> getAffiliatesMeLinksWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/v1/affiliates/me/links';
  ...
  ///
  /// Answers the caller's share links, each with its URL and its funnel: clicks tracked, signups — orgs attributed with that code — and conversions, meaning how many of those signups have actually produced commission.  Signups and conversions are DERIVED from the commission ledger and never stored, so they cannot drift from the money. Clicks are the one stored counter and the one that is pure vanity.  Any pending public click pings are folded into the store before the read, in one batch — which is how the counters stay current without a database write per click. Scoped to the validated caller's own affiliate; a non-affiliate gets `isAffiliate:false` and the link cap.
  Future<AffiliateLinks?> getAffiliatesMeLinks() async {
    final response = await getAffiliatesMeLinksWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'AffiliateLinks',) as AffiliateLinks;
  ...
  /// Parameters:
  ///
  /// * [ApplyRequest] applyRequest (required):
  Future<Response> postAffiliatesApplyWithHttpInfo(ApplyRequest applyRequest,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/affiliates/apply';

    // ignore: prefer_final_locals
    Object? postBody = applyRequest;
  ...
  ///
  /// * [ApplyRequest] applyRequest (required):
  Future<Application?> postAffiliatesApply(ApplyRequest applyRequest,) async {
    final response = await postAffiliatesApplyWithHttpInfo(applyRequest,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  /// Parameters:
  ///
  /// * [AttributeRequest] attributeRequest (required):
  Future<Response> postAffiliatesAttributeWithHttpInfo(AttributeRequest attributeRequest,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/affiliates/attribute';
  ...
  ///
  /// * [AttributeRequest] attributeRequest (required):
  Future<Attribution?> postAffiliatesAttribute(AttributeRequest attributeRequest,) async {
    final response = await postAffiliatesAttributeWithHttpInfo(attributeRequest,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'Attribution',) as Attribution;
  ...
  /// Parameters:
  ///
  /// * [ClickRequest] clickRequest (required):
  Future<Response> postAffiliatesClickWithHttpInfo(ClickRequest clickRequest,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/affiliates/click';

    // ignore: prefer_final_locals
    Object? postBody = clickRequest;
  ...
  ///
  /// * [ClickRequest] clickRequest (required):
  Future<ClickCount?> postAffiliatesClick(ClickRequest clickRequest,) async {
    final response = await postAffiliatesClickWithHttpInfo(clickRequest,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'ClickCount',) as ClickCount;
  ...
  /// Parameters:
  ///
  /// * [HandleRequest] handleRequest (required):
  Future<Response> postAffiliatesMeHandleWithHttpInfo(HandleRequest handleRequest,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/affiliates/me/handle';

    // ignore: prefer_final_locals
    Object? postBody = handleRequest;
  ...
  ///
  /// * [HandleRequest] handleRequest (required):
  Future<HandleSet?> postAffiliatesMeHandle(HandleRequest handleRequest,) async {
    final response = await postAffiliatesMeHandleWithHttpInfo(handleRequest,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'HandleSet',) as HandleSet;
  ...
  /// Parameters:
  ///
  /// * [CreateLinkRequest] createLinkRequest (required):
  Future<Response> postAffiliatesMeLinksWithHttpInfo(CreateLinkRequest createLinkRequest,) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/affiliates/me/links';

    // ignore: prefer_final_locals
    Object? postBody = createLinkRequest;
  ...
  ///
  /// * [CreateLinkRequest] createLinkRequest (required):
  Future<LinkMint?> postAffiliatesMeLinks(CreateLinkRequest createLinkRequest,) async {
    final response = await postAffiliatesMeLinksWithHttpInfo(createLinkRequest,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'LinkMint',) as LinkMint;
